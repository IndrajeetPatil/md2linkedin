...
| `| Name | Role |` table                   | Labelled row values                    |
| `- item`                                  | `• item`                               |
| `  - nested`                              | `  ‣ nested`                           |
| `    - nested twice`                      | `    ◦ nested twice`                   |
| `> blockquote`                            | `> ` prefix removed                    |
| `    code` (indented block)               | Unicode monospace by default           |
...
The Markdown parser resolves nested emphasis before rendering. Bold and italic
styles combine inside the same span; code remains literal even inside emphasis
or a heading.

```python
convert("***very important***")  # → bold-italic Unicode
convert("**bold and *italic* inside**")  # → italic text inside bold
```

### Nested Bullet Lists

Each nesting level gets its own marker — `•`, `‣`, `◦`, `▪` — and two spaces
of output indentation. Deeper levels reuse `▪`. The parser derives nesting
from each list item's content column, so mixed marker widths and changes
between bullet and ordered lists are handled according to Markdown syntax.
Ordered markers keep their numbers and delimiter (`.` or `)`).
...
`md2linkedin` uses mistletoe to parse Markdown into a syntax tree before
rendering LinkedIn text. Indented and fenced code blocks, nested blockquotes,
reference links, list boundaries and thematic breaks are interpreted in their
document context. Reference definitions are consumed rather than copied into
the output.

GFM-style tables are rendered as labelled values rather than pipe rows:
...
LinkedIn has no native Markdown table or code-block layout. Labelled values
make table rows readable, while Unicode monospace approximates code styling
without guaranteeing column alignment.
